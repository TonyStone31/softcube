...
var
  ...
  Phase4UDXCrossMove: array of array of UInt16;

  Ph4UDPlusCrossPrun: array of byte;
  Ph4UDCentBrickPrun: array of byte;
  Ph4UDXCrossPrun: array of byte;

implementation

uses nxn_facecube, nxn_util;
...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(depth);
      WriteProgress('Phase4', fName, done, NSTATES);
      //1,9,69,409,2245,12255,63377,302639,1285035,4476289,11651777,20140249,23658546,24000834,24009916,24010000
    end; //maximum 15 moves
    fs := TFileStream.Create(TableDir + fName, fmCreate);
...
begin
  SetLength(Ph4UDXCrossPrun, NSTATES);

  for i := 0 to NSTATES - 1 do
    Ph4UDXCrossPrun[i] := $FF;
  Ph4UDXCrossPrun[0] := 0;
  done := 1;
  depth := 0;
  // Zur Erinnerung: die slicecoord. es wir hier nur der y-Teil benötigt
  // result := B_8_4 * Phase3CenterCoord(x, size div 2, a) +
  // Phase3CenterCoord(y, size div 2, a)

  // in den unteren 4 bit der orthoslice-Koordinate ist die y-slice Information

  while done <> NSTATES do
  begin
    for b := 0 to B_8_4 - 1 do
      for bo := 0 to 16 - 1 do
        for c := 0 to B_8_4 - 1 do
        begin
          for mv := fU1 to xB3 do
          begin
            if Ph4UDXCrossPrun[B_8_4 * (B_8_4 * bo + b) + c] = depth then
            begin
              if not Phase4Allowed[Ord(mv)] then
                continue;
              c1 := Phase4UDXCrossMove[c, Ord(mv)];
              case mv of
                fU1..fB3:
                begin
                  bo1 := bo;//allowed face moves do not change bo
              ...
              idx1 := B_8_4 * (B_8_4 * bo1 + b1) + c1;
              if Ph4UDXCrossPrun[idx1] = $FF then
              begin
                Ph4UDXCrossPrun[idx1] := depth + 1;
                Inc(done);
              end;
            end;
          end;//mv
        end;//c
    Inc(depth);//1,5,39,231,1273,6467,25179,60457,77252,78400
    //maximum 9 moves
  end;
end;
...
begin
  ...
  for i := 0 to B_8_4 - 1 do
    for k := 0 to 35 do
      Phase4UDXCrossMove[i, k] := n;

  for i := 0 to B_8_4 - 1 do
  begin
    fc.InvPhase3CenterCoord(i, 2, 2, U);
    for a := U to B do
    begin
      for k := 0 to 3 do
      begin
        fc.move(a, 0); // face move
        if (k <> 3) and Phase4Allowed[3 * Ord(a) + k] then
          Phase4UDXCrossMove[i, 3 * Ord(a) + k] :=
            fc.Phase3CenterCoord(2, 2, U);
      end;
    end;
  end;
  ...
  for i := 0 to B_8_4 - 1 do
  begin
    fc.InvPhase3CenterCoord(i, 2, 2, U);//2 arbitrary
    for a := U to B do
    begin
      for k := 0 to 3 do
      begin
        fc.move(a, 2);// x move
        if (k <> 3) and Phase4Allowed[3 * Ord(a) + k + 18] then
          Phase4UDXCrossMove[i, 3 * Ord(a) + k + 18] :=
            fc.Phase3CenterCoord(2, 2, U);
      end;
    end;
  end;
