...
begin
  ...
  SetLength(SymCoordRepToPh3Brick702Coord, 690);
...
begin
  ...
  WriteLnVerbose(Format('Initializing 3.9 GB of Memory...', []));
  ...
  if FileExists(TableDir + fName) then
  begin
    WriteLnVerbose(Format('Loading pruning table %s', [fName]));
    fs := TFileStream.Create(TableDir + fName, fmOpenRead);
    for i := 0 to N_SYMBRICK702COORD - 1 do
    begin
    ...
    fs.Free;
    WriteLnVerbose(Format('Done!', []));
  end
  else
  begin
    WriteLnVerbose(Format('Generating pruning table %s', [fName]));
    WriteLnVerbose(Format('This will take several hours.', []));
    for i := 0 to N_SYMBRICK702COORD - 1 do
    ...
    begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      Inc(done);
                      //WriteLnVerbose(Format('classidx: %d, by: %d, done: %d', [bx1_class,altby1,done]));
                    end;
                  end;
                end;
              end
      ...
      WriteLnVerbose(Format('depth: %d, done: %d', [depth, done]));
      WriteProgress('Phase3', fName, done, total);
      Inc(depth);
    ...
    fs.Free;
    WriteLnVerbose(Format('Pruning table %s created.', [fName]));
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(depth);
      WriteProgress('Phase3', fName, done, NSTATES);
